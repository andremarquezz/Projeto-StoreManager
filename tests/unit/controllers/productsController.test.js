...
describe("Testa a camada Products Controller", () => {
  ...
  it("Verifica que ao chamar getAll retorne o codigo 200 e os produtos", async () => {
    ...
    expect(res.json.calledWith(data)).to.be.deep.true;
  });

  it("Verifica que ao chamar getOne retorne o codigo 200 e o produto", async () => {
    sinon.stub(productsService, "getOne").resolves(fakeOneProduct);
    res.status = sinon.stub().returns(res);
    ...
    expect(res.status.calledWith(200)).to.be.true;
    expect(res.json.calledWith(fakeOneProduct)).to.be.deep.true;
  });

  it("Verifica que ao chamar registerProduct retorne o codigo 201 e o produto com ID", async () => {
    sinon.stub(productsService, "registerProduct").resolves(fakeRegister);
    res.status = sinon.stub().returns(res);
    ...
    expect(res.json.calledWith(fakeRegister)).to.be.deep.true;
  });

  it("Verifica que ao chamar productsIncludeTerm retorne o codigo 200 e o produto", async () => {
  ...
  it("Verifica que ao chamar updateProduct retorne o codigo 200 e o produto atualizado com novo nome", async () => {
  ...
  it("Verifica que ao chamar deleteProduct retorne apenas o codigo 204", async () => {
